...
class KloudClient:
    def __init__(self, access_key_id: str, session_instance: boto3.Session):
        ...
        self.resource_clients: dict = {
            "ec2": KloudEC2(session_instance),
            "rds": KloudRDS(session_instance),
            "ecs": KloudECS(session_instance),
            "elb": KloudELB(session_instance),
        }
        self.cw = KloudCloudWatch(session_instance)
        self.ce = KloudCostExplorer(session_instance)
    ...
    async def get_top_3_usage_average(self) -> dict:
        to_return = dict()
        top3_with_cost = await self.ce.get_total_cost_by_instance_with_top_3_usage()
        for resource_id in top3_with_cost['top3']:
            average_utilization = await self.cw.async_get_resource_utilization(resource_id)
